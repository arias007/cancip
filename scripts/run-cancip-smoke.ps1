...
(()=>{const p=app.plugins.plugins.cancip;if(!p)throw new Error('runtime unavailable');const old={enabled:p.settings.composerAutocompleteEnabled,corpus:p.editorAutocompleteMemoryCorpusCache,promise:p.editorAutocompleteMemoryCorpusPromise,memoryGeneration:p.editorAutocompleteMemoryGeneration,generation:p.editorAutocompleteGeneration,editorCache:p.editorAutocompleteCache,branchCache:p.editorAutocompleteBranchCache,prefetches:p.editorAutocompleteBranchPrefetches};try{p.settings.composerAutocompleteEnabled=false;p.editorAutocompleteMemoryCorpusCache={at:Date.now(),documents:[]};p.editorAutocompleteMemoryCorpusPromise=null;p.editorAutocompleteCache=new Map([['stale','value']]);p.editorAutocompleteBranchCache=new Map([['stale',['value']]]);p.editorAutocompleteBranchPrefetches=new Map([['stale',Promise.resolve(['value'])]]);const beforeGeneration=p.editorAutocompleteGeneration,beforeMemoryGeneration=p.editorAutocompleteMemoryGeneration;p.invalidateEditorAutocompleteMemoryContext();return JSON.stringify({memoryInvalidated:p.editorAutocompleteMemoryCorpusCache===null&&p.editorAutocompleteCache.size===0&&p.editorAutocompleteBranchCache.size===0&&p.editorAutocompleteBranchPrefetches.size===0&&p.editorAutocompleteMemoryGeneration>beforeMemoryGeneration&&p.editorAutocompleteGeneration===beforeGeneration})}finally{p.settings.composerAutocompleteEnabled=old.enabled;p.editorAutocompleteMemoryCorpusCache=old.corpus;p.editorAutocompleteMemoryCorpusPromise=old.promise;p.editorAutocompleteMemoryGeneration=old.memoryGeneration;p.editorAutocompleteGeneration=old.generation;p.editorAutocompleteCache=old.editorCache;p.editorAutocompleteBranchCache=old.branchCache;p.editorAutocompleteBranchPrefetches=old.prefetches}})()
'@
    $currentFileCode = @'
...
'@
    ...
    $item = [pscustomobject]@{
      ...
      editorAutocompletePartialFallback = $editorPartialTree.partialFallback
      editorAutocompletePendingWait = $editorPartialTree.pendingWait
