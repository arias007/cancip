...
const checks = [
  ...
  ["document exports use semantic Markdown/HTML or the installed exporter with a real format", source.includes("async exportDocumentAs(file: TFile, markdown: string, format: DocumentExportFormat)") && source.includes("exportDocumentWithMobileExporter(file, conversionMarkdown, format)") && source.includes("...(isMarkdownFile(file) ? { noteDrawSourcePath: file.path } : {})") && source.includes("candidate.stat.size > 0")],
  ...
  ["Mobile PDF Exporter Office previews retain visual pages and expose an editable semantic layer", source.includes("mobilePdfExporterOfficePreviewHtml(entries, bytes, warnings)") && source.includes('mpe/preview/manifest.json') && source.includes("cancip-mpe-office-pages") && source.includes("cancip-office-visual-preview") && source.includes("cancip-office-semantic-preview") && source.includes('body[data-cancip-edit-mode="true"]') && source.includes('document.body?.setAttribute("data-cancip-edit-mode","true")')],
  ...
  ["sandboxed HTML and Office previews report contextual anchors without same-origin access", source.includes('post("context-edit",{kind:selectedText.trim()?"selection":"position"') && source.includes('event.data.type === "context-edit"') && source.includes("showContextEditFramePayload(iframe, snapshot.file, event.data)") && source.includes("workbenchFrameGeometry") && !source.includes("allow-same-origin")],
