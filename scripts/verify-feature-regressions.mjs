...
const checks = [
  ...
  ["context input typing cannot re-anchor through the global keyup selection route", source.includes('const openBubble = this.contextEditBubbleEl;\n        if (openBubble?.contains(openBubble.ownerDocument.activeElement)) return;\n        const contextAnchor = this.settings.contextualEditingEnabled')],
  ["context bubble keyup stays isolated from document selection handlers", source.includes('bubble.addEventListener("keyup", stop)')],
  ["context input focus blocks stale selectionchange geometry before anchor refresh", source.includes('const bubble = this.contextEditBubbleEl;\n        if (bubble?.contains(bubble.ownerDocument.activeElement)) return;\n        if (bubble && this.contextEditBubbleSurface')],
