...
const inlineEmbedSource = source.slice(
  source.indexOf("private async processMarkdownWorkbenchEmbeds"),
...
const checks = [
  ...
  ["inline workbench previews use native two-axis touch scrolling without full-workbench gesture capture", source.includes('name="cancip-inline-workbench"') && source.includes('overflow-x:auto!important;overflow-y:visible!important') && source.includes('overscroll-behavior-x:contain!important;overscroll-behavior-y:auto!important') && source.includes('touch-action:pan-x pan-y!important') && source.includes('const inlineWorkbench=document.querySelector') && source.includes('if(!inlineWorkbench&&typeof PointerEvent==="function")') && source.includes('}else if(!inlineWorkbench){') && inlineEmbedSource.includes('frame.setAttribute("scrolling", "yes")') && styles.includes("touch-action: pan-x pan-y;")],
