...
const checks = [
  ...
  ["workbench zoom persists and exposes bounded menu, wheel, keyboard, and pinch controls", workbenchSource.includes("zoom: this.documentZoom") && workbenchSource.includes("normalizeDocumentWorkbenchZoom(value.zoom)") && workbenchSource.includes('this.plugin.t("documentZoomOut"') && workbenchSource.includes('this.plugin.t("documentZoomReset"') && workbenchSource.includes('this.plugin.t("documentZoomIn"') && workbenchSource.includes('body.addEventListener("wheel"') && workbenchSource.includes('shell.addEventListener("keydown"') && workbenchSource.includes('target.addEventListener("touchstart"') && workbenchSource.includes('target.addEventListener("touchmove"') && workbenchSource.includes("Math.hypot") && workbenchSource.includes("installDocumentZoomFrameInput") && workbenchSource.includes("DOCUMENT_WORKBENCH_ZOOM_MIN") && workbenchSource.includes("DOCUMENT_WORKBENCH_ZOOM_MAX") && styles.includes(".obcc-document-body.is-zoomed") && styles.includes("touch-action: pan-x pan-y")],
  ["workbench share invokes native file sharing without copying an Obsidian link", workbenchShareSource.includes("shareNavigator.share(fileData)") && workbenchShareSource.includes("this.app.vault.readBinary(file)") && workbenchShareSource.includes("documentShareUnavailable") && !workbenchShareSource.includes("obsidian://open") && !workbenchShareSource.includes("clipboard") && !workbenchShareSource.includes("documentShareCopied")],
  ["source Markdown reading renders the live Markdown inside the workbench without returning to the original preview", workbenchSource.includes('this.mode === "markdown-reading"') && workbenchSource.includes("private async renderMarkdownReading") && workbenchSource.includes("MarkdownRenderer.render(this.app, this.currentMarkdown(), stage, snapshot.file.path, this)") && workbenchSource.includes('this.switchDocumentMode(markdownReadingMode ? "markdown"') && workbenchSource.includes('this.addModeButton(modes, "preview"')],
  ...
  ["workbench NoteDraw horizontal coordinates follow iframe scrolling", source.includes('event.data.type === "scroll"') && source.includes("this.noteDrawScrollLeft = left") && source.includes("this.noteDrawContentWidth") && source.includes("(event.clientX - metrics.rect.left) / metrics.zoom") && source.includes("(localX + this.noteDrawScrollLeft) / width")],
