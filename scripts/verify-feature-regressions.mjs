...
const checks = [
  ...
  ["document exports use semantic Markdown/HTML or the installed exporter with a real format", source.includes("async exportDocumentAs(file: TFile, markdown: string, format: DocumentExportFormat)") && source.includes("exportDocumentWithMobileExporter(file, conversionMarkdown, format)") && source.includes("format\n      });") && source.includes("candidate.stat.size > 0")],
  ...
  ["Mobile PDF Exporter DOCX rebuilds positioned and flowed page rows without affecting ordinary DOCX", source.includes("Obsidian Mobile PDF Exporter") && source.includes("docxPositionedTextMarkdown(body)") && source.includes("docxMobileExporterFlowMarkdown(body)") && source.includes("rowHeight * index") && source.includes("split(/\\t+/)") && source.includes("gap > 600_000") && source.includes("markdownTable(tableLines[0].cells")],
