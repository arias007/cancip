...
const checks = [
  ...
  ["zoomed workbench keeps a monotonic layout extent instead of relying on transform overflow", workbenchSource.includes("private documentHorizontalExtentWidth = 0") && workbenchSource.includes("private syncDocumentHorizontalExtent(body: HTMLElement, reset = false)") && workbenchSource.includes("Math.ceil(body.clientWidth * this.documentZoom)") && workbenchSource.includes('cls: "obcc-document-horizontal-extent"') && workbenchSource.includes("this.documentHorizontalExtentWidth,") && workbenchSource.includes("this.installDocumentHorizontalExtentTracking(body)") && workbenchSource.includes("body.addEventListener(\"scroll\", () => this.syncDocumentHorizontalExtent(body)")],
