...
const checks = [
  ...
  ["background indexing stays bounded while OCR is idle-only and one-file-at-a-time", source.includes("const UNIVERSAL_SEARCH_TEXT_BUILD_BATCH = 8") && source.includes("const UNIVERSAL_SEARCH_BACKGROUND_DELAY_MS = 30000") && source.includes("scheduleUniversalSearchBackgroundOcr") && source.includes("UNIVERSAL_SEARCH_BACKGROUND_OCR_IDLE_MS") && source.includes("this.universalSearchLastUserActivityAt") && source.includes("processed = true") && source.includes("break;") && source.includes("universalSearchDocumentText(item.path, item.kind, maxTextChars, full)")],
  ...
  ["workbench restores a stable NoteDraw magic-wand action through the versioned surface API", workbenchSource.includes('this.addIconButton(actions, "wand-sparkles", this.plugin.t("documentDrawWithNoteDraw")') && workbenchSource.includes("private noteDrawPublicApi()") && workbenchSource.includes("registerSurface") && workbenchSource.includes("nativeNoteDrawSurfaceHandle") && workbenchSource.includes("noteDrawWorkbenchViewport") && workbenchSource.includes("registeredSurface.toggle") && workbenchSource.includes("runtime.syncWorkspaceControllers?.()") && workbenchSource.includes('surface.surface === "workspace"') && workbenchSource.includes("private async toggleNoteDrawWorkbenchSurface") && workbenchSource.includes("api.toggle({")],
