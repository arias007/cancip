...
const checks = [
  ...
  ["subagent cards stay visibly anchored under their launching process step", source.includes('const cards = step.createDiv({') && source.includes("obcc-process-subagent-cards") && source.includes("hydrateProcessSubagentCards") && !source.includes("data-subagent-step-id") && styles.includes(".obcc-subagent-track") && styles.includes("overflow-x: auto")],
  ...
  ["process details default folded except approval and Plan stays above the composer", source.includes('this.wireDetails(stepDetails, `process-step:${stepFoldKey}`, needsIntervention, false, true)') && !source.includes("obcc-process-step-plan-button") && !source.includes("processStepPlanReference") && source.includes("this.togglePlanMenu()")],
  ["numbered process rows show concise reasoning and next while folded detail combines action with result", source.includes('cls: "obcc-process-step-thought"') && source.includes('cls: "obcc-process-step-next"') && source.includes('chinese ? "动作与结果" : "Action & result"') && source.includes("brief.action.trim(), brief.result.trim()") && !source.includes('[chinese ? "推理摘要" : "Reasoning", brief.reasoning') && styles.includes(".obcc-process-step-thought") && styles.includes(".obcc-process-step-next")],
  ["tool steps show one combined action-result summary plus a distinct raw result", source.includes("if (!stepRuns.length) this.renderProcessStepBrief(stepBody, stepInfo.brief)") && source.includes('isChineseLanguage(this.plugin.language()) ? "动作与结果" : "Action & result"') && source.includes('isChineseLanguage(this.plugin.language()) ? "原始结果" : "Raw result"')],
  ["tool-result continuation exchanges merge into the tool step instead of creating duplicate numbered rows", source.includes("private mergeToolContinuationSteps(") && source.includes("this.mergeToolContinuationSteps(concreteSteps)") && source.includes("merged.brief = { ...previous.brief }") && source.includes("previous.elapsedMs) + Math.max(0, step.elapsedMs")],
  ["generic model placeholders disappear while their raw exchange follows the next concrete tool step", source.includes("private collapseProgrammaticModelSteps(") && source.includes("const concreteSteps = this.collapseProgrammaticModelSteps(rawSteps)") && source.includes("const merged = this.mergeProcessRecordSteps(next, step)") && source.includes("if (!next || !nextRuns.length) continue")],
  ["visible messages process summaries and tool results omit stored truncation markers", source.includes("function compactProcessText(") && source.includes("function cleanProgrammaticTruncationMarkers(") && source.includes("cleanProgrammaticTruncationMarkers(redactSensitiveText(content))") && source.includes('cleanProgrammaticTruncationMarkers(run.error || run.result || "")') && source.includes("return line ? compactProcessText(redactSensitiveText(line), 72)") && source.includes("cleanProgrammaticTruncationMarkers(stripFinalReviewMetadata(stripModelRunStatsLines(baseContent)))") && source.includes("compactProcessText(stepInfo.brief.reasoning || stepInfo.headline, 180)") && source.includes("compactProcessText(stepInfo.brief.next, 150)")],
  ...
  ["changed-file controls stay on the producing step with no floating process footer", source.includes('cls: "obcc-process-step-files-button"') && source.includes("this.openLiveFilesMenu(changedFiles)") && !source.includes("this.renderProcessRecordMeta(body, items)") && styles.includes(".obcc-process-step-files-button")],
  ["tool-step reasoning names concrete session search open and distinct subagent targets", source.includes("private commandRunReasoning(") && source.includes('command === "cancip.sessionHistory"') && source.includes('command === "cancip.searchVault"') && source.includes('command === "cancip.openFile"') && source.includes('/(?:检索|搜索|search).{0,36}(?:记忆|memor)/i.test(normalized)') && source.includes('return "PROFILE.md 身份字段"') && source.includes('return "身份与称呼记忆"')],
  ["duplicate tool steps explain that the rerun was blocked and the prior result reused", source.includes("private duplicateToolRunReasoning(") && source.includes("阻止重跑并复用前次结果") && source.includes("block the rerun and reuse the prior result")],
