...
const checks = [
  ...
  ["workbench zoom persists and exposes bounded menu, wheel, keyboard, and capture-phase pinch controls", workbenchSource.includes("zoom: this.documentZoom") && workbenchSource.includes("normalizeDocumentWorkbenchZoom(value.zoom)") && workbenchSource.includes('this.plugin.t("documentZoomOut"') && workbenchSource.includes('this.plugin.t("documentZoomReset"') && workbenchSource.includes('this.plugin.t("documentZoomIn"') && workbenchSource.includes('body.addEventListener("wheel"') && workbenchSource.includes('shell.addEventListener("keydown"') && workbenchSource.includes('target.addEventListener("touchstart"') && workbenchSource.includes('target.addEventListener("touchmove"') && workbenchSource.match(/passive: false, capture: true/g)?.length >= 4 && workbenchSource.includes("Math.hypot") && workbenchSource.includes("installDocumentZoomFrameInput") && workbenchSource.includes("DOCUMENT_WORKBENCH_ZOOM_MIN") && workbenchSource.includes("DOCUMENT_WORKBENCH_ZOOM_MAX") && styles.includes(".obcc-document-body.is-zoomed") && styles.includes("touch-action: pan-x pan-y")],
  ["workbench zoom keeps NoteDraw controls fixed while content, drawings, and text share visible coordinates", workbenchSource.includes('const content = stage.createDiv({ cls: "obcc-document-zoom-content" })') && workbenchSource.includes('body.querySelectorAll<HTMLElement>(".obcc-document-zoom-content, .obcc-document-editor, .obcc-document-note-draw-edit-surface")') && !workbenchSource.includes("this.applyDocumentZoomSurface(stage)") && workbenchSource.includes("this.nativeNoteDrawController?.resizeCanvas?.()") && workbenchSource.includes("stage.scrollWidth") && workbenchSource.includes("stage.scrollHeight") && styles.includes(".obcc-document-zoom-content") && styles.includes(".obcc-document-zoom-content > .obcc-document-preview-host")],
