...
const checks = [
  ...
  ["Markdown file embeds reuse the workbench preview while ordinary links stay untouched", source.includes("registerMarkdownPostProcessor") && source.includes("processMarkdownWorkbenchEmbeds") && !source.includes('"a.internal-link"') && !source.slice(source.indexOf("private async processMarkdownWorkbenchEmbeds"), source.indexOf("private async reviewGateVaultStateFingerprint")).includes("isMarkdownFile(file)") && source.includes('doc.createElement("audio")') && source.includes('doc.createElement("video")')],
