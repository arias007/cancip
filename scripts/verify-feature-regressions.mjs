...
const checks = [
  ...
  ["context edit tracks live anchors and supports native reading view", source.includes("startContextEditAnchorTracking") && source.includes("refreshContextualEditAnchorGeometry") && source.includes('anchor.domElement?.closest<HTMLElement>(".markdown-preview-view")') && !source.includes('containingLeaf?.view instanceof MarkdownView && element.closest(".markdown-preview-view, .markdown-rendered")')],
  ["NoteDraw text editing reuses Cancip contextual edits and autocomplete without changing NoteDraw", source.includes('api.registerTextIntegration("cancip", integration)') && source.includes("attachLegacyNoteDrawController(") && source.includes("this.openNoteDrawContextEdit(payload)") && source.includes('`${before}<CURSOR>${after}`') && source.includes("scheduleLegacyNoteDrawCompletion(state)") && source.includes("this.clearNoteDrawTextIntegration()")],
  ["workbench NoteDraw surfaces delegate format-aware persistence without a duplicate editor", source.includes("api.attachTextSurface({") && source.includes("wrapLegacyNoteDrawPersistence(state)") && source.includes('controller.surfaceType = "webview"') && source.includes("runtime.scheduleTextSaveNow = (file, originalText, editedText, target)") && source.includes("await this.plugin.persistDocumentPreviewTextEdit(snapshot, originalText, editedText)") && source.includes("installNoteDrawDraftBridge(stage, snapshot)") && !source.includes("dataset.cancipNoteDrawOriginal") && source.includes("this.plugin.releaseLegacyNoteDrawController(this.nativeNoteDrawController)")],
  ...
  ["context edit renders a green Markdown proposal at the source anchor", source.includes("renderContextEditInlinePreview") && source.includes("MarkdownRenderer.render(this.app, markdown, preview, anchor.file.path, this)") && source.includes("obcc-context-edit-inline-rendered-preview") && styles.includes(".obcc-context-edit-inline-rendered-preview") && styles.includes("var(--color-green)")],
