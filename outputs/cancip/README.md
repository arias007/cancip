...
- Automatic same-Vault LAN sync supplements Remotely Save with authenticated discovery, AES-GCM encrypted transfers, write preconditions, and conflict copies for ordinary notes and attachments. Remotely Save continues its cloud/deletion work unchanged; while an authenticated LAN peer is active, Cancip can temporarily show a green Wi-Fi progress item in the same status-bar position and restores Remotely Save's prior visibility exactly afterward. Plugin/config folders, hidden roots, trash, and deletions are excluded from LAN propagation.
