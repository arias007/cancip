...
- Native File Explorer pinning: pin/unpin from the normal file context menu, keep pinned files above ordinary sibling files, reorder pinned siblings by compact move actions or an explicit drag-handle mode, and persist folder-local order in `.cancip/file-pins.json` for Vault sync without replacing Obsidian's normal unpinned sort or file drag/move behavior.
...
- Built-in local automation templates for non-desktop agent tasks are seeded from source for new users: review-gate package generation, local capability-pack import, lightweight local version snapshots, GitHub status checks, vault index refresh, a daily read-only Vault checkup with concrete health signals/friend-like reminders, and a unified Vault curation task whose automatic lane scans only manually created new files once while existing files/folders remain available through the explicit strong-scope Skill.
